  ...
  <div class="omat-tiedot mb-4">
      ...
      <div class="d-flex">
        ...
        <div class="flex-fill">
          ...
          >
            <template>
              <div
                v-for="yliopistoErikoisalat in form.kayttajanYliopistot"
    ...
    <div v-else>
      <b-form @submit.stop.prevent="onSubmit">
        ...
        <elsa-form-group :label="$t('nimike')">
          <template v-slot="{ uid }">
        ...
        <div v-if="$isKouluttaja()">
          <hr />
          <div
            v-for="(yliopistoErikoisalat, index) in form.kayttajanYliopistot"
            :key="yliopistoErikoisalat.yliopisto.id"
          >
            <hr v-if="index > 0" />
            <elsa-form-group :label="$t('yliopisto')">
              <template v-slot="{ uid }">
                <elsa-form-multiselect
                  :id="uid"
                  v-model="form.kayttajanYliopistot[index].yliopisto"
                  :options="yliopistotOptions(form.kayttajanYliopistot[index].yliopisto)"
            ...
            <elsa-form-group :label="$t('erikoisala')">
              <template v-slot="{ uid }">
                <elsa-form-multiselect
                  :id="uid"
                  v-model="form.kayttajanYliopistot[index].erikoisalat"
...
<script lang="ts">
  ...
  import {
    ElsaError,
    OmatTiedotLomakeKouluttajaVastuuhenkilo,
    Yliopisto,
    Kayttajatiedot,
  ...
  export default class OmatTiedotKouluttajaVastuuhenkilo extends Mixins(validationMixin) {
    ...
    form: OmatTiedotLomakeKouluttajaVastuuhenkilo = {
      nimike: null,
      email: null,
      phoneNumber: null,
      avatar: null,
    ...
    initForm(): OmatTiedotLomakeKouluttajaVastuuhenkilo {
      return {
        ...
        kayttajanYliopistot: this.kayttajaTiedot?.kayttajanYliopistot || []
      }
    }
    ...
    validateKayttajanYliopistoState(index: number) {
      const { $dirty, $error } = this.$v.form?.kayttajanYliopistot?.$each[index]?.yliopisto
        .id as Validation
      return $dirty ? ($error ? false : null) : null
    }

    validateKayttajanYliopistoErikoisalaState(index: number) {
      const { $dirty, $error } = this.$v.form?.kayttajanYliopistot?.$each[index]
        ?.erikoisalat as Validation
      return $dirty ? ($error ? false : null) : null
    }

    addYliopistoErikoisala() {
      this.form.kayttajanYliopistot.push({
    ...
    deleteYliopistoErikoisala(index: number) {
      if (this.form.kayttajanYliopistot) {
        Vue.delete(this.form.kayttajanYliopistot, index)
      }
    }
