export enum ELSA_ROLE {
  ErikoistuvaLaakari = 'ROLE_ERIKOISTUVA_LAAKARI',
  ErikoistuvaLaakariImpersonated = 'ROLE_ERIKOISTUVA_LAAKARI_IMPERSONATED',
  Kouluttaja = 'ROLE_KOULUTTAJA',
  Koulutusdekaani = 'ROLE_KOULUTUSDEKAANI',
  Lahikouluttaja = 'ROLE_LAHIKOULUTTAJA',
  OpintohallinnonVirkailija = 'ROLE_OPINTOHALLINNON_VIRKAILIJA',
  Paakayttaja = 'ROLE_PAAKAYTTAJA',
  PalvelujarjestelmanVirkailija = 'ROLE_PALVELUJARJESTELMAN_VIRKAILIJA',
  TekninenPaakayttaja = 'ROLE_TEKNINEN_PAAKAYTTAJA',
  Vastuuhenkilo = 'ROLE_VASTUUHENKILO'
}
